...
class TripAgents:
    ...
    def make_forecaster(self):
        return Agent(
            ...
            goal="""Find the city type, tier and demands of people residing there. Then Forecast the demand (number of units) for each selected model for the upcoming days, according to the past year sales data and the city type.
            . Make sure the sum of all forecasted units is strictly equal to the quantity sold during this period last year""",
            verbose=True,
            memory=True,
            backstory=(
                "Expert selecting most relevant products acording to city demands . Expert in forecasting the demand of selected products for the upcoming days in an electronics store based on city demands. The forecasted demand will help the store in managing inventory and planning marketing strategies. "
            ),
            llm=llm,
            allow_delegation=False
        )
